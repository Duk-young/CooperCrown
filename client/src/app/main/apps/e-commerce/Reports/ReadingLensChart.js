...
const ReadingLensChart = (props) => {
  ...
  useEffect(() => {
    ...
    newData.datasets[0].label = 'Reading Lens'

    orders.forEach((order) => {
      const month = order?.orderDate.toDate().getMonth();
      order.eyeglasses.map((pair) => {
        if (pair?.lensType === 'read') newData.datasets[0].data[month] += 1;
        return null
      })
    });
  ...
  return (
    <div className="w-full p-4">
      <Widget2 title="Reading Lens" data={data} />
    </div>
  );
};
export default ReadingLensChart;
