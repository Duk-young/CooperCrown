...
const SunglassesChart = (props) => {
  ...
  useEffect(() => {
    ...
    newData.datasets[0].label = 'Sunglasses'

    orders.forEach((order) => {
      const month = order?.orderDate.toDate().getMonth();
      order.eyeglasses.map((pair) => {
        if (pair?.lensColour) newData.datasets[0].data[month] += 1;
        return null
      })
    });
  ...
  return (
    <div className="w-full p-4">
      <Widget2 title="Sunglasses" data={data} />
    </div>
  );
};
export default SunglassesChart;
