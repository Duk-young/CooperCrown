...
const FlatTopLensChart = (props) => {
  ...
  useEffect(() => {
    ...
    newData.datasets[0].label = 'Flat Top Lens'

    orders.forEach((order) => {
      const month = order?.orderDate.toDate().getMonth();
      order.eyeglasses.map((pair) => {
        if (pair?.lensType === 'fTop') newData.datasets[0].data[month] += 1;
        return null
      })
    });
...
export default FlatTopLensChart;
