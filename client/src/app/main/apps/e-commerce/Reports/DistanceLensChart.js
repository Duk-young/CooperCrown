...
const DistanceLensChart = (props) => {
  ...
  useEffect(() => {
    ...
    newData.datasets[0].label = 'Distance Lens'

    orders.forEach((order) => {
      const month = order?.orderDate.toDate().getMonth();
      order.eyeglasses.map((pair) => {
        if (pair?.lensType === 'distance') newData.datasets[0].data[month] += 1;
        return null
      })
    });
...
export default DistanceLensChart;
