	...
	return (
		<Card className="w-full rounded-8 shadow-none border-1">
			<div className="p-16 px-4 flex flex-row items-center justify-between">
				<Typography className="h1 px-12">{props?.title || 'Top campaigns'}</Typography>
			...
			<table className="simple clickable">
				<thead>
					<tr>
						<th aria-label="title" />
						<th className="text-right">{props?.secondColumn || 'Clicks'}</th>
						<th className="text-right">{props?.thirdColumn || 'Conv'}</th>
					</tr>
				</thead>
				<tbody>
